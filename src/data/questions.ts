...
// 获取随机问题
export const getRandomTruth = (excludeIds: number[] = []): any => {
  const available = truthQuestions.filter(q => !excludeIds.includes(q.id));
  if (available.length === 0) return null;
  return available[Math.floor(Math.random() * available.length)];
};

export const getRandomDare = (excludeIds: number[] = []): any => {
  const available = dareChallenges.filter(d => !excludeIds.includes(d.id));
  if (available.length === 0) return null;
  return available[Math.floor(Math.random() * available.length)];
};
