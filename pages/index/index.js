...
Page({
  ...
  onLoad: function () {
    const savedHistory = wx.getStorageSync('gameHistory');
    if (savedHistory) {
      try {
        this.setData({
  ...
  handleDraw: function () {
    const { selectedMode, history } = this.data;
    if (!selectedMode) {
      wx.showToast({
        title: '请先选择模式',
        icon: 'none'
    ...
    const available = selectedMode === 'truth'
      ? truthQuestions.filter(q => !history.includes(q.id))
      : dareChallenges.filter(d => !history.includes(d.id));
    ...
    wx.setStorageSync('gameHistory', JSON.stringify(newHistory));

    const historyDetail = wx.getStorageSync('gameHistoryDetail') ? JSON.parse(wx.getStorageSync('gameHistoryDetail')) : [];
    historyDetail.push({
      mode: selectedMode,
      content: newResult.content,
      timestamp: Date.now()
    });
    wx.setStorageSync('gameHistoryDetail', JSON.stringify(historyDetail));
